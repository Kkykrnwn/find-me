...
function scaleCanvasToFitScreen() {
    ...
    const scaleX = screenWidth / 628;
    const scaleY = screenHeight / 628;

    // Gunakan skala terkecil supaya semuanya muat di layar
    const scale = Math.min(scaleX, scaleY);

    canvas.style.transform = `scale(${scale})`;

    // Tengahin canvas secara horizontal dan vertical
    const offsetX = (screenWidth - 628 * scale) / 2;
    const offsetY = (screenHeight - 628 * scale) / 2;
...
// Jaga agar tetap responsif saat orientasi berubah
window.addEventListener("resize", scaleCanvasToFitScreen);
