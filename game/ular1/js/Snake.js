class Snake extends Phaser.Scene {
    ...
    gameOver() {
        ...
        this.setGameOverText(
            this.strings.ui.gameover,
            "",
            this.score + ` ${this.strings.ui.points}`,
            "",
            ""
        );
    }
    ...
    up() {
        if (this.prev.y == 0)
            this.next = new Phaser.Geom.Point(0, -this.GRID_SIZE);
    }
    ...
    checkWallCollision() {
        this.wallGroup.children.each(wall => {
            if (
                wall.active &&
                this.snakeHead.x === wall.x &&
                this.snakeHead.y === wall.y
            ) {
                this.die();
                return;
            }
    ...
    repositionApple(apple) {
        ...
        let x = this.getRandomLocationWithin(
            this.WALL_LEFT + 1,
            this.WALL_RIGHT
        );
        let y = this.getRandomLocationWithin(
            this.WALL_TOP + 1,
            this.WALL_BOTTOM
        );
        ...
        while (!foundLocation) {
            if (
                this.locationHasCollisionWithGroup(
                    x * this.GRID_SIZE,
                    y * this.GRID_SIZE,
                    this.snakeBodyGroup
                )
    ...
    setScoreText(scoreString) {
        if (scoreString.length < this.MAX_SCORE.toString().length) {
            let spacesToAdd =
                this.MAX_SCORE.toString().length - scoreString.length + 1;
            scoreString = Array(spacesToAdd).join(" ") + scoreString;
        }
        this.addTextToGrid(this.scoreX - scoreString.length, this.scoreY, [
            scoreString
        ]);
    }
    ...
    setGameOverText(gameOverString, spacing, gameOverPointsString) {
        this.hideControls();
        this.addTextToGrid(this.OVER_X, this.OVER_Y, [
            gameOverString,
            spacing,
            gameOverPointsString
        ]);
    }
        ...
        for (let y = this.WALL_TOP; y <= this.WALL_BOTTOM; y++) {
            for (let x = this.WALL_LEFT; x <= this.WALL_RIGHT; x++) {
                if (
                    ...
                    let wall = this.wallGroup.create(
                        x * this.GRID_SIZE,
                        y * this.GRID_SIZE,
                        "wall"
                    );
    ...
    createTextGrid() {
        ...
        for (let y = 0; y < this.NUM_ROWS; y++) {
            ...
            for (let x = 0; x < this.NUM_COLS; x++) {
                ...
                const char = this.add.bitmapText(
                    charX,
                    charY,
                    "atari",
                    "",
    ...
    createInstructions() {
        ...
        if (this.sys.game.device.os.desktop) {
            this.addTextToGrid(
                instructionsX,
                instructionsY,
                this.strings.ui.reset.keyboard,
            ...
            this.addTextToGrid(
                instructionsX,
                instructionsY,
                this.strings.ui.reset.touch.restart,
                this.instructionsGroup,
                this.instructionsButtonGroup,
                this.restart
            );
            this.addTextToGrid(
                instructionsX + 9,
                instructionsY,
                this.strings.ui.reset.touch.menu,
                this.instructionsGroup,
                this.instructionsButtonGroup,
                this.gotoMenu
            );
    ...
    createSnake() {
        ...
        this.snakeHead = this.add.image(
            this.SNAKE_START_X * this.GRID_SIZE,
            this.SNAKE_START_Y * this.GRID_SIZE,
            "head"
        );
    ...
    createInput() {
        if (this.sys.game.device.os.desktop) {
            ...
            this.input.on("pointerup", () => {
                const dx =
                    this.input.activePointer.upX -
                    this.input.activePointer.downX;
                const dy =
                    this.input.activePointer.upY -
                    this.input.activePointer.downY;
