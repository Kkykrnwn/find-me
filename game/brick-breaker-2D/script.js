...
const Text = {
    text: 'Klik Untuk Memulai Permainan',
    draw: function (str = '') {
        ctx.clearRect(0,
            (CANVAS_HEIGHT / 2)
            + 20, canvas.width, 100);
...
function init() {
    ...
    document.getElementById("curr_score")
        .innerText = `SKOR KAMU : ${CURRENT_SCORE}`
    document.getElementById("high_score")
        .innerText = `SKOR TERTINGGI : ${HIGH_SCORE}`
    ball.draw();
    paddle.draw();
    obstacle.createGrid();
    obstacle.create();
...
function x() {
    ...
    Text.draw('Game dimulai dalam 3 detik!');
